...
public class MainActivity extends Activity {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					...
					for (String key : keys) {
						...
						writeToTextView(DATA_LINE_PADDING + StringUtils.padRight(result.getParserName(), minKeyLength)
	...
	// private InputStream getAssetStream(String urlString) throws IOException {
	// URL url = new URL(urlString);
	// HttpURLConnection conn = (HttpURLConnection) url.openConnection();
	// conn.setReadTimeout(1000000 /* milliseconds +00 */);
	// conn.setConnectTimeout(1500000 /* milliseconds +00 */);
	...
	private long test(final TestJson testJson, int repeats,String suffix, int deers) {
		long duration = 0;
		InputStream inputStream = null;
		// try {
		inputStream = getAssetStream(deers+"deer_"+repeats+"positions"+suffix+".json");
		// } catch (IOException e) {
		// return duration;
		// }
	...
	private void testImpl(final TestJson testJson, Map<String, ResultsContainer> results) {
		...
		int runs = 10;
		int deers = 1;
		String suffix = "_unformatted";
		long duration = test(testJson, runs,suffix,deers);
		results.put(StringUtils.padLeft(runs, 5) +suffix+"_"+deers+"deers_" + testJson.getName(), new ResultsContainer(testJson.getName(),
				duration, runs));
//		runs = 10;
//		deers = 1;
//		suffix = "_unformatted";
//		duration = test(testJson, runs,suffix,deers);
//		results.put(StringUtils.padLeft(runs, 5) +suffix+"_"+deers+"deers_" +  testJson.getName(), new ResultsContainer(testJson.getName(),
//				duration, runs));
		runs = 50;
		deers = 1;
...
//				duration, runs));
	...
	// try {
	// inputStream =
	// getAssetStream("http://zver.agris.cz/cs/homepage/show-last-positions?deerids=12108-12112&limit=250&type=points&period=W&num=2&xml=2");
	// }
	// catch (IOException e){
	// return;
	// }
